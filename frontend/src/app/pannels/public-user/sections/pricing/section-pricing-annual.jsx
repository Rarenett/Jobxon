  ...
  fetch("http://127.0.0.1:8000/api/pricing-plans/")
    ...
    .then(data => {
      ...
      const monthlyPlans = data.filter(
        plan => plan.frequency.trim().toLowerCase() === "annual"
      );
          ...
          >
            <div
              className={`pricing-table-1 ${
                plan.recommended ? "circle-yellow" : index === 2 ? "circle-pink" : ""
              }`}
            >
              {plan.recommended && <div className="p-table-recommended">Recommended</div>}
                  ...
                  <ul>
                    {plan.features.map((feature, i) => (
                      <li key={i} className={feature.available ? "" : "disable"}>
                        <i className={`feather-${feature.available ? "check" : "x"}`} />
                        {feature.feature}
                      </li>
                    ))}
                  </ul>
