...
class EmployerJobSerializer(serializers.ModelSerializer):
    """Serializer for employer-side job posting"""
    category_name = serializers.CharField(source='category.name', read_only=True)
    job_type_name = serializers.CharField(source='job_type.name', read_only=True)
    posted_by_name = serializers.CharField(source='posted_by.username', read_only=True)
    company_name = serializers.CharField(source='company.name', read_only=True)
    ...
    class Meta:
        ...
        fields = [
            ...
            'location', 'latitude', 'longitude', 'email', 'website', 'est_since',
            'complete_address', 'description', 'start_date', 'end_date',
            'posted_by_name', 'created_at', 'updated_at', 'slug'
        ]
        read_only_fields = ['slug', 'created_at', 'updated_at', 'posted_by', 'company_name']
        ...
        if request and hasattr(request, "user"):
            ...
            try:
                ...
                raise serializers.ValidationError({
                    "error": "You must have a company profile to post jobs."
                })
